...
selected_features = ['estimated_per_node_peak_memory','statement']
...
estimated_per_node_peak_memory = df['estimated_per_node_peak_memory'].values.reshape(-1, 1)
...
kmeans.fit(estimated_per_node_peak_memory)

# Replace the 'bytes_streamed' column with cluster labels
df['estimated_per_node_peak_memory_cluster'] = kmeans.labels_

# Explore the cluster statistics
cluster_stats = df.groupby('estimated_per_node_peak_memory_cluster')['estimated_per_node_peak_memory'].describe()
print(cluster_stats)
...
y = df['estimated_per_node_peak_memory_cluster']
...
model_node = XGBRegressor()
model_node.fit(X_train, y_train)

joblib.dump(model_node, 'node_cluster_model.bin')
joblib.dump(tfidf_vectorizer, 'node_tfidf_vectorizer.bin')
