...
selected_features = ['thread_cpu_time','statement']  #
...
thread_cpu_time = df['thread_cpu_time'].values.reshape(-1, 1)
...
kmeans.fit(thread_cpu_time)

# Replace the 'duration_seconds' column with cluster labels
df['cpu_cluster'] = kmeans.labels_

# Explore the cluster statistics
cluster_stats = df.groupby('cpu_cluster')['thread_cpu_time'].describe()
print(cluster_stats)
...
#X_duration = df['duration_seconds'].values.reshape(-1, 1)
...
joblib.dump(model_cpu, 'cpu_cluster_model.bin')
joblib.dump(tfidf_vectorizer, 'cpu_tfidf_vectorizer.bin')
