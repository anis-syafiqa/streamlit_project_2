...
selected_features = ['bytes_streamed','statement']  #
df = df[selected_features]

df.dropna(inplace = True)

# Select the 'duration_seconds' column
bytes_streamed = df['bytes_streamed'].values.reshape(-1, 1)
...
kmeans.fit(bytes_streamed)

# Replace the 'bytes_streamed' column with cluster labels
df['bytes_streamed_cluster'] = kmeans.labels_

# Explore the cluster statistics
cluster_stats = df.groupby('bytes_streamed_cluster')['bytes_streamed'].describe()
print(cluster_stats)
...
model_bytes = XGBRegressor()
model_bytes.fit(X_train, y_train)

joblib.dump(model_bytes, 'bytes_cluster_model.bin')
joblib.dump(tfidf_vectorizer, 'bytes_tfidf_vectorizer.bin')

# Make predictions
y_pred = model_bytes.predict(X_test)
